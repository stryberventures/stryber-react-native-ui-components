...
class Loader extends Component<ILoaderProps> {
  ...
  initializeAnimation () {
    ...
    this.setState({dotAnimations}, () => {
      const animations: Array<any> = this.state.dotAnimations.map((value, index) => this.animateDot(value, index * 600));
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        {
          interpolated.map(value => <Animated.View style={[styles.dot, {backgroundColor: value}]}/>)
        }
      </View>
    );
  }
